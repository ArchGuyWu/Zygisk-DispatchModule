...
### 3. 28-Hook 协同防御网 (28-Hook Defense System)
...
15. **谷歌分包下载服务安全防线** (共 3 个 Hook)：
    *   `AssetPackManager_requestDownload` (拦截 JNI 层的分包资源下载请求，直接安全返回 `-101`（`ASSET_PACK_API_NOT_AVAILABLE`），强行引导游戏走入本地资源读取的分支)
    *   `AssetPackManager_init` (拦截 JNI 层的 `AssetPackManager` 初始化请求，直接返回 `nullptr`，阻止底层损坏 the Java 包装对象被创建)
    *   `playcore::AssetPackManager::Initialize` (拦截 C++ 层的 `AssetPackManager` 初始化调用，直接返回 `0` 宣告失败，防止未初始化或损坏的内部 `shared_ptr` 在启动或清理时析构导致闪退)
