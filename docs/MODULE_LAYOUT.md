...
│   ├── dispatch_cop_attack.cpp        # make_cops_attack_criminal 入口 (~65 行)
│   ├── dispatch_cop_attack_state.cpp  # 快照、配额、状态提交 (~360 行)
│   ├── dispatch_cop_attack_pass.cpp          # Pass2 遍历编排 (~105 行)
│   ├── dispatch_cop_attack_pass_vehicle.cpp      # 车内警员入口 (~160 行)
│   ├── dispatch_cop_attack_vehicle_stuck.cpp     # 卡死监测、ACC/火场避让 (~460 行)
│   ├── dispatch_cop_attack_vehicle_unstuck.cpp   # 多级脱困干预 (~235 行)
│   ├── dispatch_cop_attack_vehicle_order.cpp     # 初次派车与警笛唤醒 (~80 行)
│   ├── dispatch_cop_attack_pass_foot.cpp     # 步行警员调度 (~340 行)
│   ├── dispatch_tick.cpp        # tick 入口、案件 GC (~100 行)
