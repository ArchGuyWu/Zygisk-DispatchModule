...
static void hide_post_load_transition_overlay(const char* via);
static void try_settle_gs9_overlay(const char* via);
...
fn_UGameterface_RunningDeferredLoadingScreenFade_t g_running_deferred_loading_screen_fade =
    nullptr;
fn_UGameterface_StartLoadingTransition_t g_start_loading_transition = nullptr;
fn_CGameLogic_Update_t g_orig_cgame_logic_update = nullptr;
void* g_stub_ccamera_run_deferred_fade_if_bound = nullptr;
void* g_stub_ccamera_fade = nullptr;
void* g_stub_running_deferred_loading_screen_fade = nullptr;
void* g_stub_cgame_logic_update = nullptr;
fn_UGameterface_RunningDeferredLoadingScreenFade_t
    g_orig_running_deferred_loading_screen_fade = nullptr;

static std::atomic<void*> g_menu_gameterface_self{nullptr};
static std::atomic<bool> g_deferred_fade_in_kick{false};
static std::atomic<int> g_fade_in_kick_count{0};
static std::atomic<int64_t> g_last_fade_in_kick_ms{0};
static std::atomic<bool> g_game_state_nudge_attempted{false};
...
static std::atomic<bool> g_post_load_fade_recovered{false};
...
static std::atomic<bool> g_loading_screen_fade_out_done{false};

static constexpr int kMaxFadeInKickAttempts = 1;
static constexpr int kGs9OverlayMaxAttempts = 8;
static constexpr int kGs9OverlayForceCompleteAttempt = 5;
...
static constexpr int64_t kGameState0StallNudgeMs = 400;
static constexpr int64_t kGameState8To9StallMs = 3000;
static constexpr int64_t kGameState8To9StreamingFallbackMs = 8000;

void notify_menu_gameterface_self(void* self) {
    if (!self || !is_pointer_readable(self)) return;
    g_menu_gameterface_self.store(self, std::memory_order_release);
}

void* get_menu_gameterface_self() {
    void* self = g_menu_gameterface_self.load(std::memory_order_acquire);
    if (self && is_pointer_readable(self)) return self;
    return nullptr;
}
...
void notify_menu_read_save_path(const char* via) {
    ...
    g_game_state_nudge_attempted.store(false, std::memory_order_release);
    ...
    g_post_load_fade_recovered.store(false, std::memory_order_release);
    ...
    g_loading_screen_fade_out_done.store(false, std::memory_order_release);
...
static int16_t read_screen_fade_status() {
    if (!g_TheCamera || !is_pointer_readable(g_TheCamera) || !g_get_screen_fade_status) {
        return -1;
    }
...
static bool is_menu_save_load_path() {
    const uint8_t kind = g_save_load_kind.load(std::memory_order_acquire);
    return kind == static_cast<uint8_t>(SaveLoadKind::MenuGenericLoad) ||
...
// Fade(1,1) darken (log 031708). Skip orig only — do not mutate transition state.
static bool should_skip_in_generic_load_fade_zero() {
...
static void* get_active_transition_screen() {
    if (!g_get_transition_manager) return nullptr;
    void* transition = g_get_transition_manager();
    if (!transition || !is_pointer_readable(transition)) return nullptr;
    void** screen_slot =
        reinterpret_cast<void**>(reinterpret_cast<char*>(transition) + 0x28);
    if (!is_pointer_readable(screen_slot)) return nullptr;
    void* screen = *screen_slot;
    if (!screen || !is_pointer_readable(screen)) return nullptr;
    return screen;
}

// RE GetScreenFadeStatus: screen+0x288==0 → returns 0; Fade(0,0) waits on screen+0x2a1.
static void preclear_fade_wait_state(void* screen) {
    if (!screen || !is_pointer_readable(screen)) return;
    if (g_ui_transition_screen_force_fade_opacity) {
        g_ui_transition_screen_force_fade_opacity(screen, 0.0f);
    }
    uint8_t* busy = reinterpret_cast<uint8_t*>(reinterpret_cast<char*>(screen) + 0x2a1);
    if (is_pointer_readable(busy)) {
        *busy = 0;
    }
    uint8_t* fade_gate = reinterpret_cast<uint8_t*>(reinterpret_cast<char*>(screen) + 0x288);
    if (is_pointer_readable(fade_gate)) {
        *fade_gate = 0;
    }
    uint8_t* fade_aux = reinterpret_cast<uint8_t*>(reinterpret_cast<char*>(screen) + 0x2a8);
    if (is_pointer_readable(fade_aux)) {
        *fade_aux = 0;
    }
}
...
static bool force_complete_post_load_fade(void* camera, const char* via, bool aggressive) {
    ...
    if (acted) {
        ...
        LOGW("💾 [SaveLoad] Force fade complete — fadeStatus %d→%d via %s aggressive=%d",
...
static void try_nudge_game_state_loading_started_ex(const char* via, bool immediate) {
    ...
    if (cleared <= 0) return;
    if (!immediate && now_ms() - cleared < kGameState0StallNudgeMs) return;
    ...
    *g_cgame_logic_game_state = kGameStateLoadingStarted;
    ...
    g_game_state_9_nudge_attempted.store(false, std::memory_order_release);
    LOGW("💾 [SaveLoad] gameState %s nudge 0→8 via %s — fadeStatus=%d streaming=%d",
...
static void try_nudge_game_state_gameplay_idle_ex(const char* via, bool immediate) {
    ...
    if (!g_post_load_fade_recovered.load(std::memory_order_acquire)) return;
    uint8_t game_state = 0;
    if (!read_game_state(&game_state) || game_state != kGameStateLoadingStarted) return;
...
// Continue_Base here left partial overlay (log 143714 — edges/bottom sliver visible).
static void hide_post_load_transition_overlay(const char* via) {
    void* screen = get_active_transition_screen();
    if (screen) {
        preclear_fade_wait_state(screen);
    }
    LOGI("💾 [SaveLoad] Hide loading overlay — preclear via %s", via ? via : "?");
}

static void try_run_loading_screen_fade_out_chain(const char* via) {
    void* self = get_menu_gameterface_self();
    if (!self || !g_orig_running_deferred_loading_screen_fade) return;
    if (!g_loading_screen_fade_out_done.exchange(true, std::memory_order_acq_rel)) {
        g_orig_running_deferred_loading_screen_fade(self, 1.0f, 1, false);
        LOGI("💾 [SaveLoad] Loading HUD fade-out (1,1) at gs9 via %s", via ? via : "?");
    }
    g_orig_running_deferred_loading_screen_fade(self, 0.0f, 0, false);
    LOGI("💾 [SaveLoad] Loading HUD dismiss (0,0,0) at gs9 via %s", via ? via : "?");
}

static void try_settle_gs9_overlay(const char* via) {
    if (!is_menu_save_load_path()) return;
    if (!g_save_load_session.load(std::memory_order_acquire)) return;
    if (g_gs9_overlay_settled.load(std::memory_order_acquire)) return;
    uint8_t game_state = 0;
    ...
    if (before > 0 && g_TheCamera && is_pointer_readable(g_TheCamera)) {
        if (attempt < kGs9OverlayForceCompleteAttempt) {
            if (g_orig_ccamera_run_deferred_fade_if_bound) {
                g_orig_ccamera_run_deferred_fade_if_bound(g_TheCamera);
            }
            if (g_orig_ccamera_fade) {
                g_orig_ccamera_fade(g_TheCamera, 1.0f, 0);
            }
        } else {
            force_complete_post_load_fade(g_TheCamera, via, true);
            hide_post_load_transition_overlay(via);
            try_run_loading_screen_fade_out_chain(via);
        }
    } else if (attempt >= kGs9OverlayForceCompleteAttempt) {
        hide_post_load_transition_overlay(via);
        try_run_loading_screen_fade_out_chain(via);
    }
    const int16_t after = read_screen_fade_status();
...
static void mark_post_load_fade_recovered(const char* via) {
    if (g_post_load_fade_recovered.load(std::memory_order_acquire)) return;
    g_post_load_fade_recovered.store(true, std::memory_order_release);
    g_post_load_fade_recovered_ms.store(now_ms(), std::memory_order_release);
    g_deferred_fade_in_kick.store(false, std::memory_order_release);
    LOGI("💾 [SaveLoad] Post-load fade recovered — allow natural Fade(0,0) via %s "
         "fadeStatus=%d streaming=%d",
         via ? via : "?",
         static_cast<int>(read_screen_fade_status()),
         read_streaming_num_requested());
}
...
static void kick_post_load_fade_in_once(const char* via) {
    ...
    if (attempts >= kMaxFadeInKickAttempts) {
...
static void run_deferred_fade_in_kick_if_needed(const char* via) {
    if (!g_deferred_fade_in_kick.load(std::memory_order_acquire)) return;
    try_kick_fade_in_after_black(via);
}

// Orig DeferredFade(0,0,0) hangs at gs0 (log 124752); safe at gs9 (log 135931 split-screen).
static void try_dismiss_deferred_loading_screen(const char* via, bool /*from_fade_complete*/) {
    ...
    if (g_post_load_fade_recovered.load(std::memory_order_acquire)) {
        ...
        if (!read_game_state(&game_state) || game_state != kGameStateIdle) {
            LOGI("💾 [SaveLoad] Defer loading HUD dismiss until gs9 via %s", via);
            return;
        }
        try_run_loading_screen_fade_out_chain(via);
    ...
    if (is_load_fade_tail_active()) {
        g_loading_hud_dismissed.store(true, std::memory_order_release);
        LOGI("💾 [SaveLoad] Dismiss loading HUD — load-tail mark via %s fadeStatus=%d",
             via,
             static_cast<int>(read_screen_fade_status()));
        return;
    }
    ...
    if (should_replace_post_load_deferred_fade_zero()) {
        LOGW("💾 [SaveLoad] Dismiss loading HUD — defer fade-in kick via %s", via);
...
static void try_kick_fade_in_after_black(const char* via) {
    ...
    kick_post_load_fade_in_once(via);
}
...
static void maybe_log_load_transition_heartbeat(const char* via) {
    ...
    last_ms = now;
    LOGI("💾 [SaveLoad] Heartbeat %s — gameState=0 ms_bLoading=%d sinceCleared=%lldms "
         "streaming=%d fadeStatus=%d",
         via,
...
static void notify_generic_load_returned(int slot, bool load_ok) {
    ...
    if (!ms_loading) {
        ...
        if (is_menu_save_load_path()) {
            if (read_screen_fade_status() >= 1 &&
                !g_post_load_fade_recovered.load(std::memory_order_acquire)) {
                g_deferred_fade_in_kick.store(true, std::memory_order_release);
                run_deferred_fade_in_kick_if_needed("GenericLoad return");
            } else if (read_screen_fade_status() <= 0) {
                mark_post_load_fade_recovered("GenericLoad return");
            }
        }
    } else {
...
void notify_ue_menu_load_committed(const char* via, int slot) {
    ...
    const bool have_game_state = read_game_state(&game_state);
    const bool ms_loading = read_ms_b_loading();
    LOGI("💾 [SaveLoad] Continue via %s(slot=%d) — ms_bLoading=%d gameState=%u",
         via,
         slot,
         ms_loading ? 1 : 0,
         have_game_state ? game_state : 255u);
...
static void session_timing_thresholds(int64_t* min_active_ms, int64_t* dwell_ms) {
    ...
    } else if (kind == static_cast<uint8_t>(SaveLoadKind::Continue)) {
        // Continue: relaxed settle + moderate hold — long session blocked touch HUD (log 114519).
        *min_active_ms = 8000;
        *dwell_ms = 3000;
    } else {
...
void poll_save_load_hydration_state() {
    ...
    if (have_game_state &&
        ...
        begin_save_load_session();
        vanilla_qol_on_gameplay_idle_entered();
    }
    ...
    if (ms_loading || (have_game_state && game_state == kGameStateLoadingStarted)) {
        ...
        g_ms_b_loading_cleared_ms.store(now_ms(), std::memory_order_release);
        if (is_menu_save_load_path() && !is_continue_load_path() &&
            have_game_state && game_state == 0) {
            begin_save_load_session();
        }
    ...
    if (have_game_state && game_state == 0) {
        ...
        const int64_t since_8 =
            g_game_state_8_since_ms.load(std::memory_order_acquire);
        const int64_t now = now_ms();
        if (since_8 > 0 && now - since_8 >= kFadeInKickPollMs) {
        ...
        try_settle_gs9_overlay("pollGs9");
        last_fade_stall_diag_ms = 0;
    } else {
        last_fade_stall_diag_ms = 0;
    }
...
// ManageTasks execution: hard-stop only during post-load hydration (session + gameState 9).
// ms_bLoading and gameState 7/8 must run ManageTasks or the loading screen hangs.
static inline bool is_manage_tasks_execution_paused() {
    if (read_ms_b_loading()) return false;
    if (!g_save_load_session.load(std::memory_order_acquire)) return false;
    uint8_t game_state = 0;
    if (!read_game_state(&game_state) || game_state != kGameStateIdle) {
    ...
    if ((kind == static_cast<uint8_t>(SaveLoadKind::MenuGenericLoad) ||
        ...
        post_load_spawn_streaming_ready()) {
        return false;
    }
    return true;
}
...
inline void* simplest_in_task_chain_readonly(void* task, size_t subtask_offset = 0x10) {
    ...
    for (int depth = 0; depth < 32; ++depth) {
        ...
        if (!sub || task_slot_unsafe_for_vtable_call(sub, 0x18)) return current;
        current = sub;
    }
    return current;
}

using TaskGetIdFn_t = int (*)(void*);
...
// RE: FindActiveTaskByType calls vtable+0x28 GetId per slot — no CALL_PREV (#33, fault 0x28).
inline void* find_active_task_by_type_readonly(void* task_mgr, int type) {
    if (!task_mgr || !is_pointer_readable(task_mgr)) return nullptr;
    for (int i = 0; i < 11; ++i) {
        void** task_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(task_mgr) + i * 8);
        if (!is_pointer_readable(task_slot)) continue;
        void* task = *task_slot;
...
inline void* simplest_active_task_from_mgr_readonly(void* task_mgr) {
    if (!task_mgr || !is_pointer_readable(task_mgr)) return nullptr;
    for (int i = 0; i < 5; ++i) {
        void** task_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(task_mgr) + i * 8);
        if (!is_pointer_readable(task_slot)) continue;
...
static void sanitize_manage_tasks_pre_call(void* self, bool force) {
    ...
    for (int i = 0; i < 11; ++i) {
        ...
        void* task = *task_slot;
        if (!task) continue;
        if (task_chain_walk_unsafe(task, 0x28)) {
...
void proxy_manage_tasks(void* self) {
    ...
    if (!self || !is_pointer_readable(self)) return;
    if (is_load_transition_engine_fastpath()) {
        run_deferred_fade_in_kick_if_needed("ManageTasks pre");
        if (g_post_load_fade_recovered.load(std::memory_order_acquire)) {
            advance_post_load_after_recovery("ManageTasks");
        }
        maybe_log_load_transition_heartbeat("ManageTasks");
        SHADOWHOOK_CALL_PREV(proxy_manage_tasks, self);
        run_deferred_fade_in_kick_if_needed("ManageTasks post");
        try_kick_fade_in_after_black("ManageTasks post");
        return;
    }
    if (is_stability_sanitize_paused()) {
    ...
    sanitize_manage_tasks_pre_call(self, false);
    SHADOWHOOK_CALL_PREV(proxy_manage_tasks, self);
}
...
void* proxy_find_active_task(void* self, int type) {
    ...
    if (!fastpath && !is_stability_sanitize_paused()) {
        ...
        void* result = SHADOWHOOK_CALL_PREV(proxy_find_active_task, self, type);
        if (!result || !is_task_vtable_safe(result)) {
            if (result) {
                LOGW("⚠️ [FindActiveTaskByType] unsafe task %p — returning nullptr", result);
            }
            return nullptr;
        }
        return result;
...
static inline bool ped_speech_playback_ready(void* ped) {
    ...
    void* intel = *p_intel;
    if (!is_pointer_readable(intel) || intelligence_zero_filled(intel)) return false;
    ...
    if (!is_pointer_readable(speech) || !is_task_vtable_safe(speech)) return false;
    return ped_field_range_readable(speech, 0x1c, 32);
}
...
void proxy_process_static_counter(void* self) {
    ...
    if (!is_stability_sanitize_paused()) {
        sanitize_task_manager_slots(reinterpret_cast<char*>(self) + 8, "ProcessStaticCounter", 0x18);
    }
    SHADOWHOOK_CALL_PREV(proxy_process_static_counter, self);
}
...
// --- CCamera fade hooks (menu load darkening) ---
void proxy_ccamera_run_deferred_fade_if_bound(void* self) {
    SHADOWHOOK_STACK_SCOPE();
    uint8_t game_state = 0;
    const bool have_game_state = read_game_state(&game_state);
    if (is_engine_load_transition_active()) {
        LOGI("💾 [SaveLoad] CCamera::RunDeferredFadeIfBound self=%p gameState=%u",
             self, have_game_state ? game_state : 255u);
    }
    if (g_orig_ccamera_run_deferred_fade_if_bound) {
...
void proxy_ccamera_fade(void* self, float duration, int16_t direction) {
    ...
    if (duration == 0.0f && direction == 0 &&
        ...
        LOGI("💾 [SaveLoad] Allow Fade(0,0) at gs9 — fadeStatus=%d streaming=%d",
             static_cast<int>(read_screen_fade_status()),
             read_streaming_num_requested());
        if (g_orig_ccamera_fade) {
            g_orig_ccamera_fade(self, duration, direction);
        }
        try_settle_gs9_overlay("Fade gs9");
        return;
    }
    ...
    if (is_engine_load_transition_active()) {
        LOGI("💾 [SaveLoad] CCamera::Fade(%.2f,%d) self=%p gameState=%u",
             duration, static_cast<int>(direction), self,
             have_game_state ? game_state : 255u);
    }
    if (g_orig_ccamera_fade) {
        g_orig_ccamera_fade(self, duration, direction);
...
void proxy_running_deferred_loading_screen_fade(void* self,
    ...
    if (duration == 0.0f && direction == 0 && !flag &&
        should_replace_post_load_deferred_fade_zero()) {
        LOGW("💾 [SaveLoad] Defer DeferredFade(0,0,0) — fade-in kick via ManageTasks "
...
void proxy_cgame_logic_update() {
    SHADOWHOOK_STACK_SCOPE();
    static uint8_t prev_game_state = 255;
    static int64_t last_stall_log_ms = 0;
    uint8_t game_state = 0;
    const bool have_game_state = read_game_state(&game_state);
    if (have_game_state && game_state != prev_game_state &&
        (game_state <= kGameStateIdle || prev_game_state <= kGameStateIdle)) {
        LOGI("💾 [SaveLoad] CGameLogic::Update — gameState %u→%u ms_bLoading=%d "
             "streaming=%d fadeStatus=%d",
             prev_game_state,
             game_state,
             read_ms_b_loading() ? 1 : 0,
             ...
             static_cast<int>(read_screen_fade_status()));
        prev_game_state = game_state;
    } else if (have_game_state) {
        prev_game_state = game_state;
    }
    if (have_game_state && game_state == 0 && !read_ms_b_loading()) {
        run_deferred_fade_in_kick_if_needed("CGameLogic::Update pre");
    }
    if (g_orig_cgame_logic_update) {
...
// intel+0x58 IK table → node+0x10 → target+0x18 null → fault 0x18 (#17–19).
inline bool intelligence_ik_node_target_unsafe(void* intel) {
    if (!intel || !is_pointer_readable(intel)) return false;
    void** table_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(intel) + 0x58);
    if (!is_pointer_readable(table_slot) || !*table_slot) return false;
    void* table = *table_slot;
    if (!is_pointer_readable(table)) return true;
    void** node_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(table) + 0x10);
    if (!is_pointer_readable(node_slot) || !*node_slot) return false;
    void* node = *node_slot;
    if (!is_pointer_readable(node)) return true;
    void** target_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(node) + 0x18);
    if (!is_pointer_readable(target_slot) || !*target_slot) return true;
    return task_slot_unsafe_for_vtable_call(*target_slot, 0x18);
}

inline bool intelligence_process_hotpath_unsafe(void* intel, size_t vtable_fn_offset) {
    if (!intel || !is_pointer_readable(intel) || intelligence_zero_filled(intel)) return true;
    void* task_mgr = reinterpret_cast<char*>(intel) + 8;
    if (task_manager_has_unsafe_slot(task_mgr, 11, vtable_fn_offset)) return true;
    for (size_t off : {0x20u, 0x28u}) {
        void** task_slot = reinterpret_cast<void**>(reinterpret_cast<char*>(intel) + off);
        if (!is_pointer_readable(task_slot)) continue;
        void* task = *task_slot;
        if (!task) continue;
        if (task_slot_unsafe_for_vtable_call(task, vtable_fn_offset)) return true;
        if (task_chain_walk_unsafe(task, vtable_fn_offset)) return true;
    }
