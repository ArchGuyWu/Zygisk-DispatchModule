                ...
                if (ped_type == PED_TYPE_COP) {
                    ...
                    if (veh) {
                        if (is_vehicle_pointer_valid(veh)) {
                            ...
                            // 复合下车判定：距离接近 (32米内)，或接近且可能卡死 (行驶超6秒且在60米内)，或彻底超时 (行驶超12秒)
                            bool should_exit = (v_dist < 32.0f) || 
                            ...
                            if (should_exit) {
                                ...
                                if (already_dispatched) {
                                    ...
                                    int64_t now_time = now_ms();
                                    bool trigger_intervention = false;
                                    ...
                                    if (trigger_stage2) {
                                        ...
                                        CVector warp_pos = {
                                            ...
                                            current_pos.z + dz_v * warp_factor + 0.8f 
                                        };
                                        if (g_GetCarToGoToCoors) {
                                            g_GetCarToGoToCoors(veh, &current_pos, 4, false);
                                        }
                                        ...
                                        if (dist_v > 5.0f) {
                                            CVector nudged_pos;
                                                ...
                                                // Nudge backwards by 3.5 meters, lift by 0.6 meters
                                                nudged_pos.x = current_pos.x - f_x * 3.5f;
                                                nudged_pos.y = current_pos.y - f_y * 3.5f;
                                                nudged_pos.z = current_pos.z + 0.60f;
                                                LOGI("   +- Nudged vehicle %p BACKWARDS 3.5m and elevated 0.6m to pull away from obstacles (visible).", veh);
                                            } else {
                                                ...
                                                nudged_pos.z = current_pos.z + nz * 0.1f + 0.75f;
                                                LOGI("   +- Nudged vehicle %p FORWARD 12.0m (unseen warp) to bypass obstacles.", veh);
                                            }
...
// =====================================================================
// 🚑 [Ambulance Workaround]：移动端救护车因超长视距生成即秒删 Bug 的修复
// =====================================================================
...
static void proxy_generate_one_emergency_car(unsigned int model, CVector pos) {
    ...
    if (model == 416 && g_FindPlayerCoors) { // MODEL_AMBULANCE = 416
        CVector player_pos = g_FindPlayerCoors(0);
        ...
        if (dist > 75.0f) {
            ...
            LOGI("🚑 [Ambulance Workaround] Mobile draw distance scale applied! Original spawn dist=%.1f m, scaled to %.1f m (pos: %.1f, %.1f, %.1f)", 
                 dist, target_dist, scaled_pos.x, scaled_pos.y, scaled_pos.z);
            pos = scaled_pos;
        }
    }
...
static void proxy_script_generate_one_emergency_car(unsigned int model, CVector pos) {
    ...
    if (model == 416 && g_FindPlayerCoors) { // MODEL_AMBULANCE = 416
        ...
        if (dist > 75.0f) {
            ...
            LOGI("🚑 [Ambulance Script Workaround] Mobile draw distance scale applied! Original spawn dist=%.1f m, scaled to %.1f m (pos: %.1f, %.1f, %.1f)", 
                 dist, target_dist, scaled_pos.x, scaled_pos.y, scaled_pos.z);
            pos = scaled_pos;
        }
    }
